...
void triggerTrackerVoice(int trackIdx, int note) {
    ...
    if (trackIdx < 8) { 
        ...
        if (note >= 0) { 
             DrumSound& ds = drumSounds[drumVoiceIdx]; setDrumTable(oscDrumVoices[drumVoiceIdx], ds.waveform);
...
void setupAudio() {
  ...
  for(int i=0; i<64; i++) { for(int j=0; j<16; j++) { trackerPattern[i][j].note = -1; trackerPattern[i][j].inst = 0; trackerPattern[i][j].vol = 255; } }
  ...
  for(int i=0; i<8; i++) { oscDrumVoices[i].setTable(SIN2048_DATA); envDrumVoices[i].setADLevels(255, 0); }
...
void playFingerDrum(int btnIndex) { /* 생략 */ }
...
void updateControl() {
  ...
  if (isSequencerPlaying) {
    ...
    if (millis() - lastStepTime >= interval) {
      ...
      for(int trk=0; trk<16; trk++) { int note = trackerPattern[currentStep][trk].note; if (note != -1) triggerTrackerVoice(trk, note); } 
    }
  }
...
MonoOutput updateAudio() {
  long drumSum = 0;
  for(int i=0; i<8; i++) { drumSum += getSample(oscDrumVoices[i], drumSounds[i].waveform) * envDrumVoices[i].next(); }
